...
The most direct method is to mail mailto:security@webtide.com[security@webtide.com].
...
If the issue is related to Eclipse or its Jetty integration then we encourage you to reach out to mailto:security@eclipse.org[ecurity@eclipse.org].
...
While Webtide works diligently to make Jetty as issue-free as possible, with the rapid pace of updates to technology and standards, there is always the possibility that a bug or vulnerability can be found.
...
What follows is the process by which we address security vulnerabilities.
...
. Generate a https://www.first.org/cvss/calculator/3.0[CVE score] and add it to the advisory description.
. Identify a https://cwe.mitre.org/data/definitions/699.html[CWE Definition] and add it to the advisory description.
...
. Open an https://bugs.eclipse.org/bugs/[Eclipse Bugzilla] to get a CVE allocated. Should be opened under the _Community_ "Product" category with a "Component" of _Vulnerability Reports_. The CVE https://www.eclipse.org/projects/handbook/#vulnerability-cve[should include] the following:
.. Version(s) affected
.. CVE Score
.. CWE Identifier(s)
...
===== A Note On Releases
Jetty has a comprehensive test catalog that covers not only functionality but also standard configuration errors that users can make which may result in instability.

When a new version of Jetty is ready to be built it undergoes rigorous testing throughout the Webtide infrastructure to make sure that it both compiles and builds successfully but that it also runs without errors and functions as expected.
Webtide uses Jetty for several of its own backend servers and web applications and new builds are tested against these before a release candidate ever moves forward for public availability.

In addition to our own testing, Webtide works with several partners who have opted to test and install early release builds in their environments.
These partners test not only Jetty as a platform but install their web applications and perform security and penetration testing against these builds.

Once internal and partner testing is complete, a process that generally takes several days, the Webtide team moves forward with public releases. R
elease notes are sent out, and builds are promoted to Maven Central.
